...
cleaned_text = ''.join(char.lower() if char.isalpha()
                       or char.isspace() else ' ' for char in text)
...
top_words = sorted(word_counts.items(), key=lambda x: (
    x[1], x[0]), reverse=True)[:10]
